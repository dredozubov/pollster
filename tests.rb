...
class PollImporterTest < Test::Unit::TestCase

  def setup
    @poll_file = YAML.load_file('polls/test.yaml')
  end
  
  def test_poll_import
    assert_nothing_raised { Poll.new @poll_file }
  end

  def test_poll_length_and_title
    poll = Poll.new @poll_file
    assert_equal poll.title, 'test'
    assert_equal poll.questions.length, 5
  end

  def test_radio_parser
    poll = Poll.new @poll_file
    assert_equal poll.questions.first['type'], 'radio'
    assert_equal poll.questions.first['answers'].length, 2
    answer = poll.questions.first['answers'].first
